...
|===
|Tax Category ID (cbc:ID) |Description
...
|===
|Tax Category Name (cbc:Name) |Tax Category ID (cbc:ID) |Description
...
=== Tax category codes Egypt

The Egyptian Tax Authority (ETA) identifies a tax with a tax type, and qualifies it with a subtype that carries the exemption or the special rate. In PUF the tax type is carried on `cac:TaxCategory/cbc:ID` and the subtype on `cac:TaxCategory/cbc:Name`. Both are supplied per tax entry, so a line carrying several taxes repeats `cac:TaxSubtotal` once per tax type. A tax type must not occur twice within the same line.

A tax subtype must be supplied for every tax entry. The tax type and subtype identify an Egyptian tax completely, so `cac:TaxScheme/cbc:ID` carries no Egyptian meaning and is set to `VAT` on every entry, including the table taxes and the fees. Withholding tax is one of the tax types below, but in PUF it is carried on `cac:WithholdingTaxTotal` rather than as an ordinary `cac:TaxTotal` entry, because the authority subtracts it when calculating the line total instead of adding it.

*The most common combination is tax type T1 with subtype V009, which indicates regular VAT on a general sale.*

|===
|Tax Type (cbc:ID) |Tax Subtype (cbc:Name) |Description
...
.*Click here for a complete list of all tax types and subtypes in Egypt.*
[%collapsible]
====
|===
|Tax Type (cbc:ID) |Tax Subtype (cbc:Name) |Description
...
|`T1`
|`V002`
|Export to free areas and other areas

|`T1`
|`V003`
|Exempted good or service

|`T1`
|`V004`
|A non-taxable good or service

|`T1`
|`V005`
|Exemptions for diplomats, consulates and embassies
...
|`T1`
|`V008`
|Special Exemptions and other reasons
...
|`T4`
|`W005`
|Sums paid by cooperative societies for car transportation
...
|`T4`
|`W007`
|Discounts & grants by smoke & cement companies

|`T4`
|`W008`
|Discounts & grants by petroleum & telecom companies

|`T4`
|`W009`
|Supporting export subsidies
...
|`T4`
|`W011`
|Commission & brokerage_A_57

|`T4`
|`W012`
|Hospitals collecting from doctors
...
|`T7`
|`Ent02`
|Entertainment tax (amount)

|`T8`
|`RD01`
|Resource development fee (rate)

|`T8`
|`RD02`
|Resource development fee (amount)
...
|`T10`
|`Mn01`
|Municipality Fees (rate)

|`T10`
|`Mn02`
|Municipality Fees (amount)

|`T11`
|`MI01`
|Medical insurance fee (rate)

|`T11`
|`MI02`
|Medical insurance fee (amount)
...
|===
|Tax Category Name (cbc:Name) |Description
